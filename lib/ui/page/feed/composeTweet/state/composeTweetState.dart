...
class ComposeTweetState extends ChangeNotifier {
  ...
  Future<void> getFCMServerKey() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to fetch FCM server key');
      }
    } catch (error) {
      cprint("Add FcmServerKey in your backend");
    }
  }
  ...
  Future<void> sendNotification(FeedModel model, SearchState state) async {
    ...
    if (status) {
      await getFCMServerKey();

      if (serverToken != null && serverToken!.isNotEmpty) {
        state.filterByUsername("");

        Iterable<Match> matches = regExp.allMatches(description);
        print("${matches.length} name(s) found in description");

        await Future.forEach(matches, (Match match) async {
          String name = match.group(0)!.substring(1);
          bool isUserFound = state.userlist!.any((x) => x.userName == name);
